...
from ccxttools import (
    EXCHANGE_API_MAPPING, API_CONFIG, CcxtTools, SimplePosition, Order,
...
if __name__ == '__main__':
    ...
    for _symbol, _amount in d_coinm_balance.items():
        l_output_string.append(f'CoinM保证金持仓,{_symbol},{str(_amount)}')
    for _symbol, _amount in d_usdm_positions.items():
        l_output_string.append(f'USDM对冲合约持仓,{_symbol},{str(_amount)}')
    for _symbol, _amount in d_order.items():
        l_output_string.append(f'Spot下单,{_symbol},{str(_amount)}')
    ...
    pprint(l_output_string, indent=4)
